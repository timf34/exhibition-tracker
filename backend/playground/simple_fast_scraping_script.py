...
# -------------------- Past/exclude helpers --------------------

def looks_archive(text: str) -> bool:
    t = (text or "").lower()
    return bool(re.search(r"\b(past|archive|archives|previous|older)\b", t))

# Freeform date range parsing so we can optionally drop clearly past items
_DATE_WORDS = r"(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:t|tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)"

def try_parse_dates_freeform(s: str) -> Tuple[Optional[str], Optional[str]]:
    if not s:
        return (None, None)
    s = norm_space(s)
    # Range like "9 October 2025 - 22 February 2026" or "16 Apr – 19 Jul 2026"
    m = re.search(rf"(\d{{1,2}}\s+{_DATE_WORDS}\s+\d{{4}})\s*[-–]\s*(\d{{1,2}}\s+{_DATE_WORDS}\s+\d{{4}})", s, re.I)
    if m:
        return (m.group(1), m.group(2))
    # “1–31 January 2026”
    m = re.search(rf"(\d{{1,2}}\s*[-–]\s*\d{{1,2}}\s+{_DATE_WORDS}\s+\d{{4}})", s, re.I)
    if m:
        return (m.group(1), None)
    # “Through Oct 5, 2025”
    m = re.search(rf"(Through|Thru)\s+{_DATE_WORDS}\s+\d{{1,2}},\s*\d{{4}}", s, re.I)
    if m:
        try:
            dt = dateparse.parse(m.group(0).split()[-3:], fuzzy=True)
            day = dt.strftime('%d').lstrip('0')
            return (None, f"{day} {dt.strftime('%B %Y')}")
        except Exception:
            pass
    return (None, None)

def clearly_past(start_s: Optional[str], end_s: Optional[str]) -> bool:
    """Return True if a parseable end date is strictly in the past (UTC today)."""
    today = datetime.utcnow().date()
    try:
        if end_s:
            end = dateparse.parse(end_s, fuzzy=True).date()
            return end < today
        # If a single date is present and clearly < today, treat as past
        if start_s and not end_s:
            st = dateparse.parse(start_s, fuzzy=True).date()
            return st < today and (today - st).days > 30  # conservative
    except Exception:
        return False
    return False
...
class PageCondenser:
    ...
    def _classify_anchor(a):
        ...
        # Drop obvious archive/past links from exhibition consideration
        is_archive = looks_archive(text) or looks_archive(href)

        is_event = ("calendar" in href) or (re.search(r"\bevent(s)?\b", text))
        is_exhibition_candidate = (("exhibit" in text) or ("exhibition" in text) or ("/exhibitions" in href))
        is_exhibition = is_exhibition_candidate and not is_event and not is_archive

        is_pager = bool(re.search(r"\b(next|more|see all|load more|view all|past exhibitions|previous|archive|older)\b", text))
...
Return ONLY current or upcoming exhibitions that a visitor can click into (ignore Events, Calendar, Membership, News, Past or Archive pages).
...
Prefer the page's main heading (H1) for the title. Do not invent titles from URLs or slugs.
...
class ExhibitionsOrchestrator:
    ...
    async def _get_listing_bundle(self, museum_url: str) -> Dict[str, Any]:
        ...
        if self.follow_pagination:
            ...
            # Do NOT follow "past/archive/previous" pagination
            pagers = [a for a in pagers if not looks_archive(a.get("text","") + " " + a.get("context","")) and not looks_archive(a.get("href",""))]
            print(f"[ORCHESTRATOR] Following {len(pagers[:3])} pagination link(s)")
        ...
        # merge anchors + take combined text
        anchors, text_chunks = [], []
    ...
    async def _fetch_detail_and_extract(self, museum_name: str, href: str, timings: Dict[str, Any]) -> Optional[Exhibition]:
        ...
        async with self.semaphore:
            ...
            # Run the blocking LLM call in a thread so multiple details can run in parallel
            t1 = time.perf_counter()
            try:
                rec = await asyncio.to_thread(self.llm.extract_detail, museum_name, bundle["text"], href)
            except Exception as e:
                t_llm = (time.perf_counter() - t1) * 1000
                print(f"[DETAIL] LLM detail extraction failed: {e}")
                timings[href] = {"status":"llm_error", "error": str(e), "t_fetch_ms": round(t_fetch_total,1)}
                return None
            t_llm = (time.perf_counter() - t1) * 1000
    ...
    async def run_for_museum(self, museum_name: str, listing_url: str) -> Dict[str, Any]:
        ...
        # Dedup by href + filter out past/archive and clearly ended shows
        print(f"[MUSEUM] Step 3: Deduplicating and filtering by recency")
        seen = set(); todo = []
        for it in items:
            href = it.href
            if href in seen:
            ...
            # String-level archive filter
            text_join = f"{it.title or ''} {it.date_text or ''} {href}"
            if looks_archive(text_join):
                print(f"[MUSEUM] Skipping archive/past item (label): {it.title} ({href})")
                continue

            # Date-level filter (if LLM provided something in date_text)
            st_txt, en_txt = try_parse_dates_freeform(it.date_text or "")
            if clearly_past(st_txt, en_txt):
                print(f"[MUSEUM] Skipping clearly past item (dates): {it.title} [{st_txt} → {en_txt}]")
                continue

            seen.add(href)
            todo.append(it)

        print(f"[MUSEUM] After dedup+filter: {len(todo)} unique exhibitions to process")

        # Fetch details concurrently (LLM in threads)
...
async def main():
    ...
    # detail_concurrency limits concurrent detail fetch + LLM threads
